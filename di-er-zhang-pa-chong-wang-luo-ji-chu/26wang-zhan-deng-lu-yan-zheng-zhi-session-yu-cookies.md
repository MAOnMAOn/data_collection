...
### 5. Token
当我们的爬虫进行网站模拟登录时，有些站点仅仅传递请求头与 Cookies 并不会返回正常的登录页面，而是返回了报错信息。仔细研究之后，你也许会发现一个叫 Token 的东西，那这个 Token 又是拿来做什么的呢？

也许你可能知道了，token 应该是拿来做登录验证的，我们在代码之中传递用户名，密码(cookies)，实际上是默认网站采用基于 Session-Cookies 的安全验证机制，但这种安全验证机制，在现实的生产环境之中还存在着一些不足的(不支持跨域访问，存在着 CSRF 攻击的隐患，扩展性不高等 )。因此，越来越多的网站开始转变登录时的身份验证机制，开始使用 token 进行用户身份的验证。即在用户初始登录时提交账号数据给服务端，服务端采用一定的策略生成一个字符串（token），token字符串中包含了少量的用户信息，并且有一定的期限。服务端会把token字符串传给客户端，客户端保存token字符串，并在接下来的请求中带上这个字符串。我们这里可以把 Token 理解为一个令牌，有了这个令牌可以出入权限允许的任意地方。

![](/assets/token工作机制.png)

同时，相比于 session 认证，启用 token 认证机制可以做到：
...
作为身份认证，token 安全性比 Session 好，Session 认证只是简单的把 User 信息存储到 Session 里，因为 SID 的不可预测性，暂且认为是安全的。这是一种认证手段。 而 Token ，如果指的是 OAuth Token 或类似的机制的话，提供的是**认证**和**授权**，认证是针对用户，授权是针对 App 。其目的是让 某 App 有权利访问某用户的信息。Session 的状态是存储在服务器端，客户端只有 Session id，而 Token 的状态是存储在客户端。
